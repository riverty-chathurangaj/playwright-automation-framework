...
import { RabbitClient } from '@api-messaging/rabbit-client';
import { ConsumerHarness } from '@api-messaging/consumer-harness';
import { MessagePublisher } from '@api-messaging/publisher';
import { DLQMonitor } from '@api-messaging/dlq-monitor';
import { MessageValidator } from '@api-messaging/message-validator';
import { DatabaseClient } from '@api-database/db-client';
import { SnapshotManager } from '@api-database/snapshot-manager';
import { CleanupManager } from '@api-database/cleanup-manager';
import { GLQueryBuilder } from '@api-database/query-builder';
import { SchemaValidator } from '@api-schemas/schema-validator';
import { AIEnricher, formatAnalysisHtml } from '@shared-core/ai/ai-enricher';
...
export const test = base.extend<GLFixtures>({
  ...
  instanceId: async ({}, use: (id: number) => Promise<void>) => {
    await use(config.api.instanceId);
  },
