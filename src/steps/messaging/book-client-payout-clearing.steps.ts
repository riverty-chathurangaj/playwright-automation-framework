...
import {
  buildBookClientPayoutClearingMessage,
  type BookClientPayoutClearingMessage,
} from '@models/test-data/factories/book-client-payout-clearing.factory';
...
type BookClientPayoutClearingFixtures = {
...
When('I define a valid message for booking a client payout clearing', function (
  { store }: Pick<BookClientPayoutClearingFixtures, 'store'>,
) {
  const message = buildBookClientPayoutClearingMessage();

  store('currentMessage', message);
  store('generatedMessageId', message.messageId);
});

When('I define a message for booking a client payout clearing with the last published message id', function (
  { retrieve, store }: Pick<BookClientPayoutClearingFixtures, 'retrieve' | 'store'>,
) {
  const lastMessage = retrieve<BookClientPayoutClearingMessage>('lastPublishedMessage');
  if (!lastMessage?.messageId) {
    throw new Error('No previously published message found. Publish a message first.');
  }

  const message = buildBookClientPayoutClearingMessage({}, lastMessage.messageId);

  store('currentMessage', message);
  store('generatedMessageId', message.messageId);
});

When('I set the book client payout clearing message ID to be the same as the previous message', function (
  { retrieve, store }: Pick<BookClientPayoutClearingFixtures, 'retrieve' | 'store'>,
) {
  const currentMessage = retrieve<BookClientPayoutClearingMessage>('currentMessage');
  const lastPublished = retrieve<BookClientPayoutClearingMessage>('lastPublishedMessage');
  if (!currentMessage) {
...
When('I publish {int} book client payout clearing message(s) with an invalid message ID to {string}', async function (
  { rabbitClient, store }: Pick<BookClientPayoutClearingFixtures, 'rabbitClient' | 'store'>,
  count: number,
  exchangeLabel: string,
) {
  const { exchange, routingKey } = resolveExchange(exchangeLabel);
  const publishedMessages: BookClientPayoutClearingMessage[] = [];

  for (let i = 0; i < count; i++) {
    const invalidId = `not-a-valid-guid-${Date.now()}-${i}`;
    const message = buildBookClientPayoutClearingMessage({}, invalidId);
  ...
  logger.info(`Published ${count} book client payout clearing message(s) with invalid message IDs`);
});

// ── Database verification ────────────────────────────────────────────────────

Then('the transactions from the book client payout clearing message should exist in the database', async function (
  { dbClient, retrieve, store }: Pick<BookClientPayoutClearingFixtures, 'dbClient' | 'retrieve' | 'store'>,
) {
  const message = retrieve<BookClientPayoutClearingMessage>('lastPublishedMessage');
  if (!message) {
  ...
  // PartitionId for book-client-payout-clearing queries — speeds up Data.Transaction lookups
  ...
  logger.info('Verified book client payout clearing transactions in Data.Transaction', {
    instanceId,
    reference,
    expectedAmount,
  ...
  store('clientPayoutClearingPositiveEntry', positiveEntry);
  store('clientPayoutClearingNegativeEntry', negativeEntry);
});

Then('the book client payout clearing transactions should have the posting number {string}', function (
  { retrieve }: Pick<BookClientPayoutClearingFixtures, 'retrieve'>,
  expectedPostingNumber: string,
) {
  const positiveEntry = retrieve<Record<string, unknown>>('clientPayoutClearingPositiveEntry');
  const negativeEntry = retrieve<Record<string, unknown>>('clientPayoutClearingNegativeEntry');
