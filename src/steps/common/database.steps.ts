...
import type { SnapshotManager } from '../../database/snapshot-manager';
import type { GLQueryBuilder } from '../../database/query-builder';
...
Given('account {string} exists in the database with balance {float}', async function (
  { dbClient }: Pick<DbFixtures, 'dbClient'>,
  accountCode: string,
  balance: number,
) {
  const exists = await dbClient.accountExists(accountCode);
  expect(exists, `Account "${accountCode}" must exist in the database with balance ${balance}. Please create it via the API or seed data first.`).to.be.true;
});

Given('I capture a database snapshot of account {string}', async function (
...
Given('I capture account {string} balance', async function (
  { dbClient, store }: Pick<DbFixtures, 'dbClient' | 'store'>,
  accountCode: string,
) {
  const account = await dbClient.getAccountByCode(accountCode);
...
Then('a journal entry row should exist in the database with:', async function (
  { dbClient, queryBuilder, retrieve, currentResponse }: Pick<DbFixtures, 'dbClient' | 'queryBuilder' | 'retrieve' | 'currentResponse'>,
  dataTable: DataTable,
) {
  const expected = dataTable.hashes()[0];
  const journalId = retrieve<string>('lastCreatedId') ||
    (currentResponse.body as Record<string, unknown>)?.journalId as string;
...
Then('a journal entry row should exist in the database matching the API response', async function (
  { dbClient, currentResponse }: Pick<DbFixtures, 'dbClient' | 'currentResponse'>,
) {
...
Then('the account {string} balance should have changed by {float}', async function (
...
Then('the journal entry should have a {string} timestamp within {int} seconds of now', async function (
  ...
  const recent = await queryBuilder.verifyTimestampRecent(
    'journal_entries', 'journal_id', journalId, timestampField.replace(/([A-Z])/g, '_$1').toLowerCase(), withinSeconds,
  );
  expect(recent, `Timestamp "${timestampField}" is not within ${withinSeconds} seconds of now`).to.be.true;
});

Then('an audit trail entry should exist with:', async function (
  { dbClient, retrieve, currentResponse }: Pick<DbFixtures, 'dbClient' | 'retrieve' | 'currentResponse'>,
  dataTable: DataTable,
) {
  const expected = dataTable.hashes()[0];
  const entityId = retrieve<string>('lastCreatedId') ||
    (currentResponse.body as Record<string, unknown>)?.journalId as string;

  const auditExists = await dbClient.auditEntryExists(
    expected.entity_type,
    entityId,
    expected.action,
  );

  expect(auditExists, `Audit entry not found for entity "${expected.entity_type}" action "${expected.action}"`).to.be.true;
});

Then('an audit trail entry should exist for the journal posting', async function (
  { dbClient, currentResponse }: Pick<DbFixtures, 'dbClient' | 'currentResponse'>,
) {
  const body = currentResponse.body as Record<string, unknown>;
  const journalId = body.journalId as string;
...
Then('the audit {string} should be null', async function (
  { dbClient, currentResponse }: Pick<DbFixtures, 'dbClient' | 'currentResponse'>,
  field: string,
) {
  const body = currentResponse.body as Record<string, unknown>;
...
Then('the audit {string} should contain the persisted journal data', async function (
  { dbClient, currentResponse }: Pick<DbFixtures, 'dbClient' | 'currentResponse'>,
  field: string,
) {
  const body = currentResponse.body as Record<string, unknown>;
...
When('I query the database for trial balance totals', async function (
  { dbClient, store }: Pick<DbFixtures, 'dbClient' | 'store'>,
) {
...
Then('the sum of all debit amounts should equal the sum of all credit amounts', function (
  { retrieve }: Pick<DbFixtures, 'retrieve'>,
) {
  ...
  expect(diff).to.be.below(0.001, `Trial balance is not balanced: debits=${totals.total_debits}, credits=${totals.total_credits}`);
});

Then('the difference should be exactly {float}', function (
  { retrieve }: Pick<DbFixtures, 'retrieve'>,
  expected: number,
) {
  const totals = retrieve<{ total_debits: number; total_credits: number }>('trialBalanceTotals');
...
When('I query for journal entries with non-existent account codes', async function (
  { dbClient, store }: Pick<DbFixtures, 'dbClient' | 'store'>,
) {
...
When('I query for posted journal entries without audit trail', async function (
  { dbClient, store }: Pick<DbFixtures, 'dbClient' | 'store'>,
) {
...
Then('the result set should be empty', function (
  { retrieve }: Pick<DbFixtures, 'retrieve'>,
) {
...
Then('the trial balance totals should still be balanced', async function (
  { queryBuilder }: Pick<DbFixtures, 'queryBuilder'>,
) {
...
Then('all {int} responses should have status {int}', function (
  { retrieve }: Pick<DbFixtures, 'retrieve'>,
  count: number,
  status: number,
) {
...
Then('no duplicate journal entry IDs should exist in the database', async function (
  { queryBuilder }: Pick<DbFixtures, 'queryBuilder'>,
) {
...
Then('the final account {string} balance should equal the initial balance plus the sum of all posted amounts', async function (
  { dbClient, retrieve }: Pick<DbFixtures, 'dbClient' | 'retrieve'>,
  accountCode: string,
) {
...
When('I send {int} concurrent POST requests to {string} for account {string}', async function (
  ...
  const { journalEntryFactory } = await import('../../models/test-data/factories/journal-entry.factory' as any);
  const url = `/api/${require('../../core/config').config.apiVersion}${path}`;
