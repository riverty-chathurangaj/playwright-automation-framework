...
} from '@api-models/gl/test-data/factories/book-client-payout-clearing.factory';
...
When(
  'I define a valid message for booking a client payout clearing',
  function ({ store }: Pick<BookClientPayoutClearingFixtures, 'store'>) {
...
When(
  'I define a message for booking a client payout clearing with the last published message id',
  function ({ retrieve, store }: Pick<BookClientPayoutClearingFixtures, 'retrieve' | 'store'>) {
...
When(
  'I set the book client payout clearing message ID to be the same as the previous message',
  function ({ retrieve, store }: Pick<BookClientPayoutClearingFixtures, 'retrieve' | 'store'>) {
...
When(
  'I publish {int} book client payout clearing message(s) with an invalid message ID to {string}',
  async function (
...
Then(
  'the transactions from the book client payout clearing message should exist in the database',
  async function ({
    dbClient,
    retrieve,
    store,
  }: Pick<BookClientPayoutClearingFixtures, 'dbClient' | 'retrieve' | 'store'>) {
...
Then(
  'the book client payout clearing transactions should have the posting number {string}',
  function ({ retrieve }: Pick<BookClientPayoutClearingFixtures, 'retrieve'>, expectedPostingNumber: string) {
