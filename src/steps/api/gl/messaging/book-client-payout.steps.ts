...
} from '@api-models/gl/test-data/factories/book-client-payout.factory';
...
When(
  'I define a valid message for booking a client payout',
  function ({ store }: Pick<BookClientPayoutFixtures, 'store'>) {
...
When(
  'I define a message for booking a client payout with the last published message id',
  function ({ retrieve, store }: Pick<BookClientPayoutFixtures, 'retrieve' | 'store'>) {
...
When(
  'I set the book client payout message ID to be the same as the previous message',
  function ({ retrieve, store }: Pick<BookClientPayoutFixtures, 'retrieve' | 'store'>) {
...
When(
  'I publish {int} book client payout message(s) with an invalid message ID to {string}',
  async function (
...
Then(
  'the transactions from the book client payout message should exist in the database',
  async function ({ dbClient, retrieve, store }: Pick<BookClientPayoutFixtures, 'dbClient' | 'retrieve' | 'store'>) {
...
Then(
  'the book client payout transactions should have the posting number {string}',
  function ({ retrieve }: Pick<BookClientPayoutFixtures, 'retrieve'>, expectedPostingNumber: string) {
