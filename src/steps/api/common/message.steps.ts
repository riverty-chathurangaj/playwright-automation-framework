...
import type { CollectedMessage } from '@api-messaging/consumer-harness';
import type { RabbitClient } from '@api-messaging/rabbit-client';
import type { ConsumerHarness } from '@api-messaging/consumer-harness';
import type { DLQMonitor } from '@api-messaging/dlq-monitor';
import type { MessageValidator } from '@api-messaging/message-validator';
...
Given(
  'I am listening on {string}',
  async function ({ consumerHarness }: Pick<MessagingFixtures, 'consumerHarness'>, exchangeLabel: string) {
...
Given(
  'I am listening on the {string} exchange',
  async function ({ dlqMonitor }: Pick<MessagingFixtures, 'dlqMonitor'>, exchangeLabel: string) {
    const { exchange, routingKey } = resolveExchange(exchangeLabel);
...
When(
  'I publish the message to {string}',
  async function (
...
When(
  'I publish the message to {string} with routing key {string}',
  async function (
    { rabbitClient, retrieve, store }: Pick<MessagingFixtures, 'rabbitClient' | 'retrieve' | 'store'>,
    exchangeLabel: string,
...
When(
  'I publish the same message again to {string}',
  async function (
...
Then(
  'I should receive {int} message(s) within {int} seconds',
  async function (
...
Then(
  'no messages should be received within {int} seconds',
  async function ({ consumerHarness }: Pick<MessagingFixtures, 'consumerHarness'>, seconds: number) {
    await consumerHarness.assertNoMessages(seconds * 1000);
  },
);

Then(
  'I should not receive any additional messages within the next {int} seconds',
  async function ({ consumerHarness, store }: Pick<MessagingFixtures, 'consumerHarness' | 'store'>, seconds: number) {
...
Then(
  'there should be an error on the {string} exchange',
  async function ({ dlqMonitor, store }: Pick<MessagingFixtures, 'dlqMonitor' | 'store'>, _exchangeLabel: string) {
...
Then(
  'there should be no errors on the {string} exchange',
  async function ({ dlqMonitor }: Pick<MessagingFixtures, 'dlqMonitor'>, _exchangeLabel: string) {
    await dlqMonitor.assertNoDLQMessages(5_000);
  },
);

Then(
  'there should be {int} error(s) on the {string} exchange within {int} seconds',
  async function (
...
Then(
  'the message should match schema {string}',
  function (
    ...
    expect(
      ...
      `Message schema validation failed for "${schemaName}":\n${result.errors
        ?.map((error: { path: string; message: string | undefined }) => `  ${error.path}: ${error.message}`)
        .join('\n')}`,
    ).to.be.true;
  },
);
...
Then(
  'the message field {string} should equal {string}',
  function (
    { retrieve, messageValidator }: Pick<MessagingFixtures, 'retrieve' | 'messageValidator'>,
    field: string,
    expected: string,
...
Then(
  'the message field {string} should equal {float}',
  function (
    { retrieve, messageValidator }: Pick<MessagingFixtures, 'retrieve' | 'messageValidator'>,
    field: string,
    expected: number,
...
Then(
  'the message field {string} should match the API response {string}',
  function (
    {
      retrieve,
      messageValidator,
      currentResponse,
    }: Pick<MessagingFixtures, 'retrieve' | 'messageValidator' | 'currentResponse'>,
    msgField: string,
    apiField: string,
  ) {
    const message = retrieve<CollectedMessage>('lastMessage');
    const msgValue = messageValidator.getField(message, msgField);
    const { Comparator } = require('@api-utils/comparator');
...
Then(
  'the message header {string} should equal {string}',
  function ({ retrieve }: Pick<MessagingFixtures, 'retrieve'>, header: string, expected: string) {
    const message = retrieve<CollectedMessage>('lastMessage');
...
Then(
  'the message property {string} should not be empty',
  function (
    { retrieve, messageValidator }: Pick<MessagingFixtures, 'retrieve' | 'messageValidator'>,
    property: string,
  ) {
    const message = retrieve<CollectedMessage>('lastMessage');
    const value =
      (message as unknown as Record<string, unknown>)[property] || messageValidator.getField(message, property);
    expect(value, `Message property "${property}" should not be empty`).to.be.ok;
  },
);

Then(
  'the messages should be in chronological order',
  function ({ retrieve, messageValidator }: Pick<MessagingFixtures, 'retrieve' | 'messageValidator'>) {
...
Then(
  'each message should have a unique {string}',
  function ({ retrieve, messageValidator }: Pick<MessagingFixtures, 'retrieve' | 'messageValidator'>, idField: string) {
    const messages = retrieve<CollectedMessage[]>('lastMessages');
...
Then(
  'each message {string} should match its API response correlation',
  function ({ retrieve, messageValidator }: Pick<MessagingFixtures, 'retrieve' | 'messageValidator'>, field: string) {
    const messages = retrieve<CollectedMessage[]>('lastMessages');
...
Then(
  'the message should appear in the DLQ within {int} seconds',
  async function ({ dlqMonitor, store }: Pick<MessagingFixtures, 'dlqMonitor' | 'store'>, seconds: number) {
...
Then(
  'the DLQ message header {string} should contain routing information',
  function ({ retrieve }: Pick<MessagingFixtures, 'retrieve'>, header: string) {
...
Then(
  'the DLQ message header {string} should be {string}',
  function ({ retrieve }: Pick<MessagingFixtures, 'retrieve'>, header: string, expected: string) {
    const message = retrieve<CollectedMessage>('dlqMessage');
    const actual = message.headers[header];
    expect(String(actual)).to.equal(expected);
...
Then(
  'the message should be retried {int} times',
  async function (
...
Then(
  'after retries are exhausted the message should appear in the DLQ',
  async function ({ dlqMonitor, store }: Pick<MessagingFixtures, 'dlqMonitor' | 'store'>) {
    const message = await dlqMonitor.assertMessageInDLQ(30_000);
    store('dlqMessage', message);
...
Then(
  'the DLQ message header {string} should be {int}',
  function (
    { retrieve, messageValidator }: Pick<MessagingFixtures, 'retrieve' | 'messageValidator'>,
    header: string,
    expected: number,
