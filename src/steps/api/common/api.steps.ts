...
import { PayloadMutator, CorruptionType } from '@api-utils/payload-mutator';
import { Comparator } from '@api-utils/comparator';
import { config } from '@shared-core/config';
import { resolveStatus } from '@api-utils/http-status';
import { getTemplate, resolveEndpoint } from '@api-utils/request-templates';
...
When('I define a GET {string}', function ({ currentRequest }: { currentRequest: CurrentRequest }, requestName: string) {
...
When(
  'I define a POST {string}',
  function ({ currentRequest }: { currentRequest: CurrentRequest }, requestName: string) {
    const template = getTemplate(requestName);
...
When('I define a PUT {string}', function ({ currentRequest }: { currentRequest: CurrentRequest }, requestName: string) {
  const template = getTemplate(requestName);
...
When(
  ...
  function ({ store }: { store: (key: string, value: unknown) => void }, param: string, value: string) {
...
When(
  'I add query parameter {string} with value {string}',
  function ({ currentRequest }: { currentRequest: CurrentRequest }, key: string, value: string) {
    const parsed = !isNaN(Number(value)) && value !== '' ? Number(value) : value;
...
When(
  'I set {string} to the stored value {string}',
  function (
...
When(
  'I set field {string} to {string} in the payload',
  function ({ currentRequest }: { currentRequest: CurrentRequest }, field: string, value: string) {
...
When(
  'I remove field {string} from the payload',
  function ({ currentRequest }: { currentRequest: CurrentRequest }, field: string) {
...
When(
  'I corrupt field {string} with {string}',
  function ({ currentRequest }: { currentRequest: CurrentRequest }, field: string, corruptionType: string) {
    if (!currentRequest.body) throw new Error('No request body set');
    currentRequest.body = PayloadMutator.corruptField(currentRequest.body, field, corruptionType as CorruptionType);
  },
);
  ...
  async function (
    {
      ...
      apiClient: import('@api-core/api-client').ApiClient;
...
When(
  'I send a valid POST request to {string} with:',
  async function (
    {
      apiClient,
      currentRequest,
...
When(
  'I send an invalid POST request to {string} with:',
  async function (
    {
      apiClient,
      currentRequest,
...
Then(
  'I get the response code of {word}',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, statusLabel: string) {
...
Then(
  'the response status should be {word}',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, statusLabel: string) {
    const expected = resolveStatus(statusLabel);
    expect(
      currentResponse.status,
...
Then(
  'the response status should be {word} or {word}',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, label1: string, label2: string) {
...
Then(
  ...
  function ({ currentResponse }: { currentResponse: CurrentResponse }, field: string, expected: string) {
...
Then(
  'the response field {string} should equal {float}',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, field: string, expected: number) {
    const actual = Comparator.getNestedValue(currentResponse.body as Record<string, unknown>, field);
...
Then(
  ...
  function ({ currentResponse }: { currentResponse: CurrentResponse }, field: string) {
    const actual = Comparator.getNestedValue(currentResponse.body as Record<string, unknown>, field);
...
Then(
  'the response field {string} should be a valid UUID',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, field: string) {
    const actual = Comparator.getNestedValue(currentResponse.body as Record<string, unknown>, field);
...
Then(
  ...
  function ({ currentResponse }: { currentResponse: CurrentResponse }, field: string, allowedValues: string) {
    const actual = Comparator.getNestedValue(currentResponse.body as Record<string, unknown>, field);
...
Then('the response body should be an array', function ({ currentResponse }: { currentResponse: CurrentResponse }) {
  expect(Array.isArray(currentResponse.body)).to.be.true;
});

Then('the response should be an empty array', function ({ currentResponse }: { currentResponse: CurrentResponse }) {
...
Then(
  'the response body should be an array with at least {int} item(s)',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, count: number) {
    expect(Array.isArray(currentResponse.body)).to.be.true;
...
Then(
  'the response array should contain exactly {int} items',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, count: number) {
    expect(Array.isArray(currentResponse.body), 'Response body should be an array').to.be.true;
...
Then(
  'each item in the response array field {string} should equal {string}',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, field: string, expected: string) {
    expect(Array.isArray(currentResponse.body), 'Response body should be an array').to.be.true;
    ...
    items.forEach((item, index) => {
      ...
      expect(String(actual), `Item[${index}] field "${field}" should equal "${expected}" but got "${actual}"`).to.equal(
        expected,
      );
    });
  },
...
Then(
  'the error message should reference field {string}',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, field: string) {
...
Then(
  'the error should indicate {string}',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, expectedText: string) {
    const body = JSON.stringify(currentResponse.body).toLowerCase();
...
Then(
  ...
  function ({ currentResponse }: { currentResponse: CurrentResponse }, maxMs: number) {
...
Then(
  'the response should contain required fields:',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, dataTable: DataTable) {
...
Then(
  'the field {string} should be of type {string}',
  function ({ currentResponse }: { currentResponse: CurrentResponse }, field: string, expectedType: string) {
    const actual = Comparator.getNestedValue(currentResponse.body as Record<string, unknown>, field);
...
Then(
  ...
  function ({ currentResponse }: { currentResponse: CurrentResponse }, expectedMessage: string) {
