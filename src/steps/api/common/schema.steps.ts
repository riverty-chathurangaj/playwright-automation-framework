...
Then(
  'each item in the response array should match schema {string}',
  function (
...
Then(
  'the response should match schema {string}',
  function (
    { currentResponse, schemaValidator }: Pick<SchemaFixtures, 'currentResponse' | 'schemaValidator'>,
    schemaName: string,
  ) {
...
Then(
  'the response should NOT match schema {string}',
  function (
    { currentResponse, schemaValidator }: Pick<SchemaFixtures, 'currentResponse' | 'schemaValidator'>,
    schemaName: string,
  ) {
...
Then(
  'no new undocumented fields should be present in the response',
  function ({
    currentResponse,
    schemaValidator,
    retrieve,
  }: Pick<SchemaFixtures, 'currentResponse' | 'schemaValidator' | 'retrieve'>) {
...
Then(
  'no previously documented fields should be missing',
  function ({
    currentResponse,
    schemaValidator,
    retrieve,
...
Then(
  'no field types should have changed from the baseline',
  function ({
    currentResponse,
    schemaValidator,
    retrieve,
...
Then(
  'I have the baseline schema snapshot for {string}',
  function ({ schemaValidator, store }: Pick<SchemaFixtures, 'schemaValidator' | 'store'>, schemaName: string) {
    store('currentSchemaContext', schemaName);
    const available = schemaValidator.getAvailableSchemas();
    expect(available.includes(schemaName), `Schema "${schemaName}" not found. Available: ${available.join(', ')}`).to.be
      .true;
  },
);
