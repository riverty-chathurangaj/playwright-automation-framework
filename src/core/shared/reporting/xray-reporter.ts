...
export class XrayReporter {
  ...
  async publishResults(reportPath?: string): Promise<void> {
    const playwrightReportPath = reportPath || path.resolve(process.cwd(), 'reports/playwright-api-report.json');
    ...
    const payload = {
      ...
      info: {
        summary: `riverty-playwright-bdd GL reference pack - ${config.env} - ${new Date().toISOString().split('T')[0]}`,
        description: `Automated Playwright BDD execution via riverty-playwright-bdd | Filter: ${process.env.PLAYWRIGHT_GREP || 'all'} | Build: ${config.gitSha}`,
